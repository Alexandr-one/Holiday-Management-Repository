...
@section("content")
    ...
    <form method="post" action="{{route('password.reset.email')}}">
        ...
        <p style="text-align: left">Введите подтвержденный адрес электронной почты вашей учетной записи пользователя, и мы вышлем вам ссылку для сброса пароля.</p>
        ...
        <div class="form-floating">
            <input type="email" value="{{ session()->has('emailReset') ? session()->pull('emailReset') : " " }}" name="email" class="form-control input_reset_password" id="floatingInput" placeholder="name@example.com">
